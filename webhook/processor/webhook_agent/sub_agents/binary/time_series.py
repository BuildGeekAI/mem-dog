"""TimeSeriesAgent — Processes numeric time-indexed data series."""

from ..base import BaseSubAgent


class TimeSeriesAgent(BaseSubAgent):
    """Processes numeric time-indexed data series."""

    AGENT_TYPE = "time_series"
    AGENT_PURPOSE = "Processes numeric time-indexed data series"
    MIME_PATTERNS = ["application/x-timeseries", "application/x-time-series"]

    def _process(self, payload_json: str, data_id: str, group_context=None, payload_meta=None) -> dict:
        """Analyse time series data using Gemma 3 1B.
